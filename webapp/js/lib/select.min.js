...
(function () {
    ...
    // Disable removing elements from ui-select-choices
    var removeSelectedOwn = false;
    ...
    uis.controller('uiSelectCtrl',
        ['$scope', '$element', '$timeout', '$filter', 'uisRepeatParser', 'uiSelectMinErr', 'uiSelectConfig', '$parse', '$injector', '$window',
            function($scope, $element, $timeout, $filter, RepeatParser, uiSelectMinErr, uiSelectConfig, $parse, $injector, $window) {
                    ...
                    if (item === undefined || !item._uiSelectChoiceDisabled) {
                        ...
                        if (!item || !item._uiSelectChoiceDisabled) {
                            ...
                            if ( ctrl.selected && angular.isArray(ctrl.selected) && ctrl.selected.filter( function (selection) { return angular.equals(selection, item); }).length > 0 ) {
                                ...
                                // todo parameterize?
                                _.remove(ctrl.selected, function (selection) { return angular.equals(selection, item); });
                            ...
                            ctrl.onBeforeSelectCallback($scope, { $item: item });
                ...
                ctrl.sizeSearchInput = function() {
                    ...
                    var input = ctrl.searchInput[0],
                        ...
                        updateIfVisible = function(containerWidth) {
                            ...
                            if (inputWidth < 15) inputWidth = containerWidth;
                            ctrl.searchInput.css('width', --inputWidth+'px');
                            return true;
                        };
                    var noTags = ctrl.searchInput.parent().find('.ui-select-match-item').length == 0;
                    ctrl.searchInput.css('min-width', noTags ? '50px' : '')
                    ctrl.searchInput.css('width', '20px');
                ...
                $scope.$on('finishResize', function() {
            ...
            function($document, uiSelectConfig, uiSelectMinErr, uisOffset, $compile, $parse, $timeout) {
                ...
                return {
                    ...
                    templateUrl: function(tElement, tAttrs) {
                        ...
                        if (angular.isDefined(tAttrs.multiple) && angular.isDefined(tAttrs.withoutChoices)) {
                            return theme + "/custom-select-multiple.tpl.html";
                        }
                    ...
                    compile: function(tElement, tAttrs) {
                        ...
                        if(tAttrs.notRemoveSelected) removeSelectedOwn = false;
                        else if(typeof tAttrs.notRemoveSelected == 'undefined') removeSelectedOwn = true;
                        else removeSelectedOwn = true;
                        ...
                        return function(scope, element, attrs, ctrls, transcludeFn) {
                            ...
                            if(tAttrs.id) $select.elementId = attrs.id;
                            $select.generatedId = uiSelectConfig.generateId();
                            $select.baseTitle = attrs.title || 'Select box';
                            $select.focusserTitle = $select.baseTitle + ' focus';
                            $select.focusserId = 'focusser-' + $select.generatedId;
                            if (angular.isDefined(attrs.contextMenuOptions)) {
                                $select.menuOptions = scope.$eval(attrs.contextMenuOptions);
                            }
                            ...
                            $select.resetChoicesPosition = function () {
                                var dropdown = angular.element(element).querySelectorAll('.ui-select-dropdown'),
                                    inputPos = $select.searchInput.offset(),
                                    totalWidth = $('body').width();
                                var position;
                                if(inputPos.left + 300 > totalWidth){
                                    position = $select.searchInput.position().left + (totalWidth - (inputPos.left + 300));
                                } else {
                                    position = $select.searchInput.position().left;
                                }
                                dropdown.css('left', position + "px");
                            }

                            $select.onBeforeSelectCallback = function($scope, args){
                                $parse(attrs.onBeforeSelect)($scope, args);
                            }

                            $select.onSelectCallback = function($scope, args){
                                // todo for keyboard only
                            ...
                            $select.resetBlurTimeout = function () {
                                if (blurTimeout) {
    ...
    uis.directive('uiSelectMultiple', ['uiSelectMinErr','$timeout', function(uiSelectMinErr, $timeout) {
        return {
            ...
            controller: ['$scope','$timeout', function($scope, $timeout){
                ...
                $scope.$on('sizeSearchInput', $select.sizeSearchInput);
            ...
            link: function(scope, element, attrs, ctrls) {
                ...
                $select.removeSelected = removeSelectedOwn;
                ...
                function _handleMatchSelection(key){
                    ...
                    if(caretPosition > 1 || ($select.search.length && key == KEY.RIGHT)) return false;
                ...
                $select.searchInput.on('blur', function(e) {
                    $timeout(function() {
                        $selectMultiple.activeMatchIndex = -1;
                    });
                });
    ...
    uis.directive('uiSelectSort', ['$timeout', 'uiSelectConfig', 'uiSelectMinErr', function($timeout, uiSelectConfig, uiSelectMinErr) {
        return {
            ...
            link: function(scope, element, attrs, $select) {
                ...
                var container = element.closest('.ui-select-container');
                var positions = [];
                var droppedItemIndex, newIndex;

                function cleanCursor(){
                    _.each(positions, function(pos){
                        pos.item.removeClass(droppingAfterClassName);
                        pos.item.removeClass(droppingBeforeClassName);
                    })
                }
                ...
                var dragOverHandler = function(event){
                    ...
                    if(positions.length > 0) {
                        ...
                        var distances = _.map(positions, function (pos) {
                            return {pos: pos, distance: distance(eventPos, pos.coors)};
                        })

                        var pos = _.min(distances, function (d) {
                            return d.distance
                        }).pos;
                        ...
                        //if (!lastPos || lastPos != pos) {
                            ...
                            if(pos.position == 'left'){
                                pos.item.addClass(droppingBeforeClassName);
                                newIndex = droppedItemIndex + 1 <= pos.index ? pos.index - 1 : pos.index;
                                //console.log('move ' + droppedItemIndex + ' to the left of ' + pos.index + ", new index is " + newIndex);
                                if(newIndex < 0) newIndex = 0;
                            } else {
                                pos.item.addClass(droppingAfterClassName);
                                newIndex = droppedItemIndex >= pos.index + 1 ? pos.index + 1 : pos.index;
                                //console.log('move ' + droppedItemIndex + ' to the right of ' + pos.index + ", new index is " + newIndex);
                            }
                            lastPos = pos;
                        //}
                    }
                ...
                var dropHandler = function(event){
                    if(newIndex > -1 && droppedItemIndex != newIndex) {
                ...
                element.on('dragstart', function(event) {
                    ...
                    container.on('dragover', dragOverHandler);
                    //console.log('dragstart');
                    //container.on('drop', dropHandler);
                    _.each(container.find('.ui-select-match-item'), function(item, i){
                        item = $(item);
                        if(scope.$index != i){
                            positions.push({
                                index: i,
                            ...
                            positions.push({
                                ...
                                position: 'right',
                                coors: {
                                    x: item.offset().left + item.outerWidth(),
                                    y: item.offset().top + item.outerHeight() / 2
                                }
                            })
                        }
                ...
                element.on('dragend', function() {
                    ...
                    cleanCursor();
                    dropHandler();
                    $(document).off('dragover', cleanCursor)
                    container.off('dragover', dragOverHandler);
                });
            }
        };
    }]);
