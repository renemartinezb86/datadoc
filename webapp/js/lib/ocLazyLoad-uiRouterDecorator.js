define([
    ...
    angular.module('oc.lazyLoad.uiRouterDecorator', ['ui.router']).config(['$stateProvider', function($stateProvider) {
        ...
        function lazyBundle(module, files, templateUrl) {
            ...
            return {
                ...
                resolve: ['$templateCache', '$ocLazyLoad', '$q', '$http', function ($templateCache, $ocLazyLoad, $q, $http) {
                    lazyDeferred = $q.defer();
                    return $ocLazyLoad.load({
                        name: module,
                        files: angular.isArray(files) ? files : [files]
                    }).then(function() {
                        var template = templateUrl && $templateCache.get(templateUrl);
                        if(template) {
                            lazyDeferred.resolve(template);
                        } else if(templateUrl) {
