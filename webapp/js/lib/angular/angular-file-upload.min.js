                ...
                var FileUploader = (function () {
                    ...
                    _createClass(FileUploader, {
                        addToQueue: {
                            ...
                            value: async function addToQueue(files, options, filters) {
                                var _this = this;
                                ...
                                forEach(list, function (some /*{File|HTMLInputElement|Object}*/, i) {
                                    var temp = new FileLikeObject(some);
                                    // Sometimes we use addToQueue() with a bunch of files with different options
                                    var specificOptions = Array.isArray(options) ? options[i] : options;

                                    if (_this._isValidFile(temp, arrayOfFilters, specificOptions)) {
                                        var fileItem = new FileItem(_this, some); // We don't have to use third parameter because it'll unite fileItem with options
                                        addedFileItems.push(fileItem);
                                        _this.queue.push(fileItem);
                                        _this._onAfterAddingFile(fileItem, specificOptions);
                                    } else {
                                        var filter = arrayOfFilters[_this._failFilterIndex];
                                        _this._onWhenAddingFileFailed(temp, filter, specificOptions);
                                    }
                                });

                                if (this.queue.length !== count) {
                                    await this._waitBeforeStartUploading(addedFileItems);
                        ...
                        waitBeforeStartUploading: {
                            /**
                             * Fires right after onAfterAddingAll and wait for execution
                             * @async
                             */
                            value: async function waitBeforeStartUploading(fileItems) {}
                        },
                        onAfterAddingFile: {
                            /**
                             * Callback
                            ...
                            value: function onAfterAddingFile(fileItem, options) {}
                        },
                        ...
                        _customTransport: {
                            /**
                             * The GCS transport which supports resumable uploading
                             * @see {@link https://github.com/QubitProducts/gcs-browser-upload}
                             * @param {FileItem} item
                             * @private
                             */

                            value: function _customTransport(item) {
                                var _this = this;

                                this.customTransport({
                                    _fileUploaderContext: _this,
                                    item,
                                });
                        ...
                        _onAfterAddingFile: {
                            ...
                            value: function _onAfterAddingFile(item, options) {
                                this.onAfterAddingFile(item, options);
                            }
                        },
                        _onAfterAddingAll: {
                            /**
                        ...
                        _waitBeforeStartUploading: {
                            /**
                             * Inner method
                             */
                            value: async function _waitBeforeStartUploading(items) {
                                await this.waitBeforeStartUploading(items);
                            }
                        },
                        _onBeforeUploadItem: {
                            /**
                ...
                var FileSelect = (function (_FileDirective) {
                    ...
                    _createClass(FileSelect, {
                        ...
                        onChange: {
                            ...
                            value: async function onChange() {
                                var files = this.uploader.isHTML5 ? this.element[0].files : this.element[0];
                                var options = this.getOptions();
                                var filters = this.getFilters();

                                if (!this.uploader.isHTML5) this.destroy();
                                await this.uploader.addToQueue(files, options, filters);
                ...
                var FileDrop = (function (_FileDirective) {
                    ...
                    _createClass(FileDrop, {
                        ...
                        onDrop: {
                             ...
                             *
                             * Attention!
                             * This code is cursed, do not touch or it will turn into bug.
                             * Function has been modified in order to support uploading folder structure (just for Chrome browser)
                             */

                            value: async function onDrop(event) {
                                ...
                                async function readDirectory(entries, fileCallback, parentFolder) {
                                    const createFileObjectPromiseList = [];
                                    ...
                                    for (var i = 0; i < entries.length; i++) {
                                        ...
                                        if (e.isDirectory) {
                                            if(_.startsWith(e.name, '.'))
                                                continue
                                            else{
                                                var entry = e;
                                                if(options.createFolderCallback){
                                                    options.createFolderCallback(entry.name, parentFolder, function(folder){
                                                        var directoryReader = entry.createReader();
                                                        getAllEntries(
                                                            directoryReader,
                                                            function(es){ readDirectory(es, fileCallback, folder.id)}
                                                        );
                                                    })
                                                } else {
                                                    var directoryReader = entry.createReader();
                                                    getAllEntries(
                                                        directoryReader,
                                                        function(es){ readDirectory(es, fileCallback, parentFolder)}
                                                    );
                                                }
                                            }
                                        } else {
                                            const createFileObjectPromise = new Promise((resolve, reject) => {
                                                const onSuccess = f => {
                                                    resolve(f, parentFolder);
                                                };
                                                const onError = e => {
                                                    errorHandler(e);
                                                    // Resolve promise even if error occured to continue onDrop
                                                    // process for other, possible successfully added files
                                                    resolve();
                                                };

                                                e.file(onSuccess, onError);
                                            });

                                            createFileObjectPromiseList.push(createFileObjectPromise);
                                        }
                                    }

                                    const files = await Promise.all(createFileObjectPromiseList)

                                    if (_.size(files)) {
                                        fileCallback(files, parentFolder)
                                    }
                                }
                                ...
                                function getAllEntries(directoryReader, callback) {
                                    ...
                                    var readEntries = function () {
                                        directoryReader.readEntries(function (results) {
                                            entries = _.remove(entries, function(item){return !_.startsWith(item.name, '.')});
                                            if (!results.length) {
                                                entries.sort();
                                                callback(entries);
                                            } else {
                                ...
                                var that = this;
                                if (transfer.items) {
                                    await readDirectory(transfer.items, async function (file, parentFolder) {
                                        await that.uploader.addToQueue(file, _.merge({}, options, {parentFolder: parentFolder}), filters);
                                    })
                                } else {
                                    await this.uploader.addToQueue(transfer.files, options, filters);
                                }
                            }
                        },
                        onDragOver: {
